...
# Steady state price of tradable stocks
stock_price_ss(Π_F, RRB, m_par) = m_par.ωΠ .* Π_F ./ (RRB .- 1 .+ m_par.ιΠ) .+ 1.0

# Steady state payout to entrepreneurs
profits_E_ss(mc, Y, RRB, m_par) =
    (1.0 .- m_par.ωΠ) .* profits_F_ss(mc, Y) .+
    m_par.ιΠ .* (stock_price_ss(profits_F_ss(mc, Y), RRB, m_par) .- 1.0)
...
function labor_market_clearing_ss(
    ...
    Π_E = profits_E_ss(mc, Y, m_par.RRB, m_par)
