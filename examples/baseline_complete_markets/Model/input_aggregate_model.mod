...
F[indexes.G] = (log(G)) - (log(BgovPrime + T - RB / π * Bgov - GI))
...
# Resource constraint
F[indexes.C] = (log(Y - G - GI - I + (1.0 - 1.0 ./ A) * RL * B / π)) - (log(C))
...
# Constant ex-ante liquidity premium determines capital stock
F[indexes.K] = (log(LPXA)) - (XSS[indexes.LPXASS])

# Accounting identity determines bonds residually
F[indexes.TotalAssets] = (log(TotalAssets)) - (log(qlag * K + B))

# Consumption Euler Equation for GHH (composite consumption)
F[indexes.B] =
    (log(mutil((C - N .^ (1 + m_par.γ) / (1 + m_par.γ)), m_par))) -
    (log(m_par.β .* RB * mutil((CPrime - NPrime .^ (1 + m_par.γ) / (1 + m_par.γ)), m_par)))
