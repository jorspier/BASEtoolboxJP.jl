...
# @set! m_par.β = 0.9908030538299242;
# @set! m_par.λ = 0.04834291158825785;
# @set! m_par.ζ = 0.00014609610653492527; 
# @set! m_par.Tlev = 1.1337763204831677; 
# @set! m_par.Rbar = 0.03993280297191498; 

# # manually changed
# @set! m_par.δ_0 = 0.02;
# @set! m_par.μ = 1.1;
# @set! m_par.μw = 1.1;
# @set! m_par.Tprog = 1.0 + 0.221;
# @set! m_par.θ_π = 1.25; 
# @set! m_par.γ_Bτ = 0.0; 
...
# @set! m_par.δ_s = 0.716408827514776;
# @set! m_par.ϕ = 1.9898095517519947;
# @set! m_par.κ = 0.14194831255970647;
# @set! m_par.κw = 0.2382079399762385;
# @set! m_par.shiftΠ = 0.6991208405603;
...
# @set! m_par.Σ_n = 0.0;
# @set! m_par.ρ_R = 0.8681540544540812;
# @set! m_par.σ_Rshock = 0.002306627917745612;
# @set! m_par.θ_Y = 0.0;
# @set! m_par.γ_B = 0.1;
# @set! m_par.γ_π = -2.0810532400000175;
# @set! m_par.γ_Y = -0.44017934906602824;
# @set! m_par.ρ_Gshock = 0.9471703682671463;
# @set! m_par.σ_Gshock = 0.005960776215459719;
# @set! m_par.ρ_τ = 0.4934161405247185;
# @set! m_par.γ_Yτ = -0.9139719081293639;
# @set! m_par.ρ_P = 0.933090864259849;
...
# @set! m_par.ρ_TFP = 0.49744167550361956;     
# @set! m_par.σ_TFP = 0.02355381294385709;    
...
# @set! m_par.δ_KG = 0.025;                    # Depreciation of public capital (2.5% per quarter - 10% per year)
# @set! m_par.η_KG = 0.1;                    # Elasticity of output w.r.t public capital
# @set! m_par.ρ_GI = 1.0e-8;                    # Persistence of GI shock
# @set! m_par.σ_Auth = 40.0 * (0.01453 / m_par.GI_share)  #m_par.σ_Gshock * (0.21 / m_par.GI_share) # scale investment shock to same absolute size as consumption shock
...
#     distr_bk    = dropdims(sum(ss_full.distrSS, dims=3), dims=3)
#     wealth_mesh = vec(ss_full.n_par.mesh_b[:,:,1] .+ ss_full.n_par.mesh_k[:,:,1])
#     dist_vec    = vec(distr_bk)

#     perm           = sortperm(wealth_mesh)
#     sorted_wealth  = wealth_mesh[perm]
#     sorted_dist    = dist_vec[perm]
#     cum_dist       = cumsum(sorted_dist)
#     total_wealth   = sum(sorted_wealth .* sorted_dist)
#     top10_idx      = findfirst(x -> x >= 0.90, cum_dist)
    
#     TOP10Wshare    = top10_idx !== nothing ?
#                      sum(sorted_wealth[top10_idx:end] .* sorted_dist[top10_idx:end]) / total_wealth :
#                      0.0
...
#     catch e
#         # If the EGM or linearization crashes due to bad parameters, return heavy penalties
#         return Dict(
#             "K/Y" => 1e6,
#             "Bgov/Y" => 1e6,
#             "G/Y" => 1e6,
...
cal_dict = Dict(
    ...
    "target_moments" => Dict( # User-defined targets # these are from paper
        ...
        "T10W" => 0.58,  # Top 10% wealth share WID average
