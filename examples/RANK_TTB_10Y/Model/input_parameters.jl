function phillips_transform(κ_q, β_q)
    # function definitions
    f_q(λ) = λ^2 * β_q - (1 + β_q + κ_q) * λ + 1
    κ(λ, β) = (1 - λ) * (1 - λ * β) / λ

    # transformations
    λ_q = find_zero(f_q, (0, 1), Bisection())
...
@label @latex_label @prior @flattenable @with_kw struct ModelParameters{T}
	...
	ξ::T = 4.0    		| "xi" 			| L"\xi" 		|  Gamma(gamma_pars(4.0, 2.0^2)...) 		| false # risk aversion
	γ::T = 2.0    		| "gamma" 		| L"\gamma" 	|  Gamma(gamma_pars(2.0, 1.0^2)...) 		| false # inverse Frisch elasticity
	β::T = 0.9917425270049023 		| "beta" 		| L"\beta" 		|  Beta(beta_pars(0.99625, 0.001^2)...)  	| false # discount factor
	λ::T = 0.053362065374623756 		| "lambda"  	| L"\lambda" 	|  _  										| false # adjustment probability
	γ_scale ::T = 0.2 	| "gamma_scale" | L"\gamma_{scale}"| _  									| false # disutiltiy of labor

	# Individual income process
	ρ_h::T = 0.9815   	| "rho" 		| L"\rho" 		|  _  										| false # autocorrelation income shock
	σ_h::T = 0.135   	| "sigma" 		| L"\sigma" 	|  _  										| false # std of income shocks (steady state)
	ι::T = 1/16   		| "iota" 		| L"\iota" 		|  _  										| false # probability to return worker
	ζ::T = 0.00014244975451929335 		| "zeta" 		| L"\zeta" 		|  _  										| false # probability to become entrepreneur

	# Technological parameters
	α::T = 0.32  		| "alpha" 		| L"\alpha" 	|  _  										| false # capital share
	δ_0::T = 0.016	    | "delta" | L"\delta" 	|  _  										| false # depreciation rate
	...
	ψ::T  = 0.3     	| "psi" 		| L"\psi" 		|  _  										| false # steady-state bond to capital ratio
	τ_lev::T  = 0.57 	| "tau_lev" 	| L"\tau^L" 	|  _  										| false # steady-state income tax rate level
	τ_prog::T  = 0.6 	| "tau_pro" 	| L"\tau^P" 	|  _  										| false # steady-state income tax rate progressivity
	# no transfers, no VAT and consumption tax
	...
	RB::T = 1.0 / β | "RB" 	| L"RB"  		|  _  														| false # Nominal Interest Rate
	Rbar::T = π*(0.036798968157815665)|"Rbar"|L"\bar R"|  _  										| false # borrowing wedge in interest rate
	ASHIFT::T = π/β  | "ASHIFT" |  L"ASHIFT"  |  _  | false # borrowing wedge in interest rate
    BhhBAR::T = 2.81  | "BhhBAR" |  L"BhhBAR"  |  _  | false # previously 2.81
	...
	shiftΠ::T = 0.5 | "shiftPi" | L"\shift^{\Pi}" | Beta(beta_pars(0.5, 0.25^2)...)| false # fraction of tradable firm-profits
	...
	ρ_TFP::T = 0.9 		| "rho_TFP" 	| L"\rho_{TFP}" 	| Beta(beta_pars(0.5, 0.2^2)...)        	| true  # Pers. of TFP
	σ_TFP::T = 0.0 		| "sigma_TFP" 	| L"\sigma_{TFP}" 	| InverseGamma(ig_pars(0.001, 0.02^2)...) 	| true  # Std of TFP
	...
	θ_π::T = 1.25 		| "theta_pi" 	| L"\theta_\pi" | Normal(1.0, 0.3)                			| false  # Reaction to inflation
	θ_Y::T = 0.0	  	| "theta_Y" 	| L"\theta_y" 	| Normal(0.125, 0.05)              			| false  # Reaction to inflation

	# fiscal policy
	γ_B::T = 0.1 		| "gamma_B" 	| L"\gamma_B" 	| Gamma(gamma_pars(0.1, 0.075^2)...)      	| false  # reaction of deficit to debt
	...
	γ_GI::T = 1.0 | "gamma_GI" | L"\gamma_{GI}" | _ | false # reaction of deficit to government investment
	GI_share::T = 0.028 | "GI_share" | L"\frac{GI}{Y}" | _ | false # share of government investment in output
	ρ_GI::T = 1.0e-8 | "rho_GI" | L"\rho_{GI}" | _ | false # Pers. in government investment shock
	σ_Auth::T = 40 * (0.01453 / GI_share) | "sigma_Auth" | L"\sigma_{Auth}" | InverseGamma(ig_pars(0.001, 0.02^2)...) | false # Std of government investment shock
	δ_KG::T = 0.01 | "delta_KG" | L"\delta_{KG}" | _ | false # depreciation rate of government capital
	η_KG::T = 0.10 | "eta_KG" | L"\eta_{KG}" | _ | false # efficiency of government capital in producing public services
...
@with_kw struct NumericalParameters
	...
	ny::Int             = 22  	    # ngrid income (4 is the coarse grid used initially in finding the StE)
	nk::Int            	= 80        # ngrid illiquid assets (capital) (10 is the coarse grid used initially in finding the StE)
	...
	kmax::Float64      	= 4500.0    # gridmax capital
	mmin::Float64      	= -6.0      # gridmin bonds
	mmax::Float64      	= 3500.0    # gridmax bonds
	...
	further_compress::Bool = false   # run model-reduction step based on MA(∞) representation
...
@with_kw struct EstimationSettings
	...
	observed_vars_input::Array{Symbol, 1} 	= [:Ygrowth, :Igrowth, :Cgrowth, :N, :wgrowth, :RB,  :π] # :GIgrowth,
	...
	meas_error_input::Array{Symbol, 1} 	=  [:Ygrowth, :Igrowth, :Cgrowth, :N, :wgrowth, :RB,  :π]
	# meas_error_input::Array{Symbol, 1} 		=  [:TOP10Wshare, :TOP10Ishare, :τprog, :σ]
	#meas_error_distr::Array{InverseGamma{Float64}, 1} = []
	meas_error_distr::Array{InverseGamma{Float64}, 1} = [InverseGamma(ig_pars(0.0005, 0.001^2)...), 
	...
	estimation_start_file::String 			= "7_Saves/mcmc_start.jld2" 

	data_file::String 						= "7_Saves/GER_growth.csv"
	estimation_container_file::String 		= "7_Saves/RANK_estim_container.jld2"
	save_posterior_file::String 			= "7_Saves/RANK_chain.jld2"
