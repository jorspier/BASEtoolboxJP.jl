...
function mode_finding(sr, lr, m_par, e_set, par_start)
  @load "7_Saves/RANK_chain_orig.jld2" er_mc
  par_start = er_mc.par_final
  ...
  # OptOpt  = Optim.Options(show_trace = true, show_every = 200, store_trace = true, x_tol = e_set.x_tol,
  #                          f_tol = e_set.f_tol, iterations = e_set.max_iter_mode)

  # opti    = optimize(LL, par , e_set.optimizer, OptOpt)

  # par_final = Optim.minimizer(opti)

  par_final = er_mc.par_final
  posterior_mode = -LL(par_final)
  ...
  lr = update_model(sr, lr, m_par)   # solve new reduced model

  println("Likelihood at mode ", posterior_mode)
  ...
  if e_set.compute_hessian == true
    ...
    # func          = TwiceDifferentiable(pp -> LL(pp), par_final)
    # hessian_final = Optim.hessian!(func, par_final)
    # hessian_final = FiniteDiff.finite_difference_hessian(LL, par_final, relstep = 0.01)
    # hessian_final = FiniteDiff.finite_difference_hessian(LL, par_final, relstep = 0.005)
    hessian_final = er_mc.hessian_final
  else 
    if sr.n_par.verbose
