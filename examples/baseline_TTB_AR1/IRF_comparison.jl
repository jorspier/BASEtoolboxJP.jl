...
# Define paths
path_frontloaded = joinpath(paths["bld"], "baseline_TTB_AR1_noestim", "IRFs_frontloaded.jld2")
path_10Y    = joinpath(paths["bld"], "baseline_TTB_10Y_noestim", "IRFs_10Y.jld2")

# Load matrices, shock orders, and index structs
IRFs_front = load(path_frontloaded, "IRFs")
ids_front  = load(path_frontloaded, "indexes")
...
for var in fieldnames(typeof(ids_10Y))
    if hasproperty(ids_front, var)
        ...
        idx_front  = getfield(ids_front, var)
        
        # Ensure the variable is an assigned index (usually > 0)
        if idx_10Y isa Int && idx_front isa Int && idx_10Y > 0 && idx_front > 0
            IRFs_front_aligned[idx_10Y, :, :] = IRFs_front[idx_front, :, :]
        end
    end
end
...
IRFs_to_plot = [
    (IRFs_front_aligned, "Front-loaded AR(1)"),
...
plot_irfs(
    ...
    horizon = 80,
    save_fig = true,
    path = joinpath(paths["bld"], "IRFs_comparison"),
